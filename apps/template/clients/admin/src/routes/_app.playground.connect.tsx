...
import type { GetMessageResponse } from "#/services/connect/template/api/v1/message_pb"
import {
  createMessage,
  deleteMessage,
  getMessage,
  queryMessage,
  updateMessage,
} from "#/services/connect/template/api/v1/message-MessageService_connectquery"
...
function ConnectPlayground() {
  ...
  const deleteMut = useMutation(deleteMessage, {
...
const UpdateDrawer = function UpdateDrawer({
  ...
  const { data } = useQuery(getMessage, { id }, { enabled: !!id })
...
const DetailModal = function DetailModal({
  ...
  const { data, isFetching } = useQuery(getMessage, { id }, { enabled: !!id })
