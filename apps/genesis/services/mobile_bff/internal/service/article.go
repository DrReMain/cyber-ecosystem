...
type ArticleService struct {
	genesisV1.UnimplementedMobileArticleServiceServer
	log       *log.Helper
...
func (s *ArticleService) RegisterGRPC(srv *grpc.Server) {
	genesisV1.RegisterMobileArticleServiceServer(srv, s)
}
func (s *ArticleService) RegisterHTTP(srv *http.Server) {
	genesisV1.RegisterMobileArticleServiceHTTPServer(srv, s)
}
func (s *ArticleService) RegisterConnect(srv *connect.Server) {
	srv.Register(genesisV1connect.NewMobileArticleServiceHandler(s, srv.HandlerOptions()...))
}
...
func (s *ArticleService) QueryArticle(ctx context.Context, in *genesisV1.QueryArticleRequest) (*genesisV1.QueryArticleResponse, error) {
	...
	return &genesisV1.QueryArticleResponse{
		...
		List: utils.SliceMap(out.List, articleToProto),
	}, nil
}

// region[rgba(144,164,174,0.10)] ⚪ Private

func articleToProto(a *biz.Article) *genesisV1.GetArticleResponse {
