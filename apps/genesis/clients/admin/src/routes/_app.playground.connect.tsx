...
import { z } from "zod"
import { buildConnectPage, buildOrderBy, defaultKV } from "#/lib/builder"
...
import {
  ...
  queryArticle,
  updateArticle,
} from "#/services/connect/genesis/api/v1/bff_admin/admin_article-AdminArticleService_connectquery"
import type { GetArticleResponse } from "#/services/connect/genesis/api/v1/messages/article_pb"

const schema = z.object({
  pageNo: pageNoField(),
  pageSize: pageSizeField(),
  sort: sortField("sort:desc"),
...
function ConnectPlayground() {
  ...
  const { data, isFetching, refetch } = useQuery(
    queryArticle,
    {
      page: { ...buildConnectPage(search), all: false },
  ...
  const deleteMut = useMutation(deleteArticle, {
...
const UpdateDrawer = function UpdateDrawer({
  ...
  const { data } = useQuery(getArticle, { id }, { enabled: !!id })
  ...
  const updateMut = useMutation(updateArticle, {
    onSuccess: () => {
      setId("")
  ...
  return (
    ...
    >
      <Form
        ...
        layout="vertical"
        onFinish={(values) =>
          updateMut.mutate({ id, ...values, fieldsMask: ["title", "content"] })
        }
      >
        <Form.Item label="标题" name="title" rules={[{ required: true }]}>
          <Input placeholder="请输入标题" />
...
const DetailModal = function DetailModal({
  ...
  const { data, isFetching } = useQuery(getArticle, { id }, { enabled: !!id })
