...
import {
  adminArticleServiceCreateArticle,
  adminArticleServiceDeleteArticle,
  adminArticleServiceGetArticle,
  adminArticleServiceQueryArticle,
  adminArticleServiceUpdateArticle,
} from "#/services/openapi/sdk.gen"
import type { ApiGenesisV1GetArticleResponse } from "#/services/openapi/types.gen"
...
function HttpPlayground() {
  ...
  const { data, isFetching, refetch } = useQuery({
    queryKey: ["queryArticle", query],
    queryFn: async () => {
      const res = await adminArticleServiceQueryArticle({
        query,
        throwOnError: true,
      })
      return res.data
  ...
  const deleteMut = useMutation({
    mutationFn: (id: string) =>
      adminArticleServiceDeleteArticle({ path: { id }, throwOnError: true }),
      ...
      <Card>
        <Flex gap={16} vertical>
          <TableToolbar
            ...
            size={tableSize}
          />
          <Table<ApiGenesisV1GetArticleResponse>
            columns={[
...
const CreateDrawer = function CreateDrawer({
  ...
  const createMut = useMutation({
    mutationFn: (values: { title: string; content: string }) =>
      adminArticleServiceCreateArticle({ body: values, throwOnError: true }),
    onSuccess: () => {
      setOpen(false)
...
const UpdateDrawer = function UpdateDrawer({
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const res = await adminArticleServiceGetArticle({
        path: { id },
        throwOnError: true,
      })
      return res.data
