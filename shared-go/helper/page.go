...
// QueryPaginator is the ent query-builder surface pagination runs against.
...
// ApplyPagination applies req's page/size/all onto an ent query and returns the
// total + offset + limit. Count runs on a clone so interceptors appending
// predicates in-place don't mutate the original builder.
func ApplyPagination[Q QueryPaginator[Q]](ctx context.Context, query Q, req *commonv1.PageRequest, config PageConfig, ce *errors.Error) (total, offset, limit int, err error) {
	if ce == nil {
...
func BuildPageResponse(total, offset, limit int) *commonv1.PageResponse {
	if limit == 0 {
