...
// Target represents a parsed connect target.
type Target struct {
	Scheme    string
	Authority string
	Endpoint  string
	...
	go func() {
		for {
			...
			if wErr != nil {
				...
				log.Error("[connect resolver] watch error: " + wErr.Error())
...
func (r *resolver) update(services []*registry.ServiceInstance) bool {
	...
	for _, ins := range services {
		...
		if err != nil {
			log.Error("[connect resolver] parse endpoint error: " + err.Error())
			continue
		}
		if ept == "" {
	...
	if len(nodes) == 0 {
		log.Warn("[connect resolver] zero endpoint found, service=" + r.target.Endpoint)
		return false
	}
