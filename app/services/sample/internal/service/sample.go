...
type SampleService struct {
	...
	log      *slog.Logger
	sampleUC *biz.SampleUC
}

func NewSampleService(logger *slog.Logger, sampleUC *biz.SampleUC) *SampleService {
	return &SampleService{
		log:      logger.With("module", "service/sample"),
		sampleUC: sampleUC,
	}
}
...
func (s *SampleService) CreateSample(ctx context.Context, in *samplepb.CreateSampleRequest) (*samplepb.CreateSampleResponse, error) {
	created, err := s.sampleUC.Create(ctx, &biz.Item{
		Name: in.Name,
	})
	if err != nil {
		return nil, err
...
func (s *SampleService) ListSamples(ctx context.Context, in *samplepb.ListSamplesRequest) (*samplepb.ListSamplesResponse, error) {
	out, err := s.sampleUC.List(ctx, &biz.ItemListIn{
...
func (s *SampleService) itemToProto(a *biz.Item) *samplepb.Sample {
