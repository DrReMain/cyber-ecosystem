...
func (s *TransferService) RegisterGRPC(srv *kratosgrpc.Server) {
	pb.RegisterTransferServiceServer(srv, s)
}

func (s *TransferService) RegisterHTTP(srv *kratoshttp.Server) {
	pb.RegisterTransferServiceHTTPServer(srv, s)
}

func (s *TransferService) RegisterConnect(srv *connecttransport.Server) {
	pb.RegisterTransferServiceConnectServer(srv, s)
}

// Handler ---------------------------------------------------------------------------------------------------------------

func (s *TransferService) Subscribe(in *pb.SubscribeRequest, stream grpc.ServerStreamingServer[pb.SubscribeResponse]) error {
	...
	for i := 0; i < 5; i++ {
		msg := &pb.SubscribeResponse{
			...
			EventType: "message",
			Data:      []byte(fmt.Sprintf("event %d on topic %q", i+1, in.GetTopic())),
...
func (s *TransferService) Raw(ctx context.Context, in *pb.RawRequest) (*httpbody.HttpBody, error) {
