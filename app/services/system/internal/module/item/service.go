...
func NewItemService(logger *slog.Logger, itemUC *ItemUC) *ItemService {
	return &ItemService{
		log:    logger.With("module", "module/item_service"),
		itemUC: itemUC,
	}
}
...
func (s *ItemService) CreateItem(ctx context.Context, in *systempb.CreateItemRequest) (*systempb.CreateItemResponse, error) {
	created, err := s.itemUC.Create(ctx, &Item{
		Name:        in.Name,
...
func (s *ItemService) UpdateItem(ctx context.Context, in *systempb.UpdateItemRequest) (*systempb.UpdateItemResponse, error) {
	if _, err := s.itemUC.Update(ctx, in.FieldsMask, &Item{
		ID:          in.Id,
		Name:        in.Name,
		Description: in.Description,
	}); err != nil {
...
func (s *ItemService) ListItems(ctx context.Context, in *systempb.ListItemsRequest) (*systempb.ListItemsResponse, error) {
	out, err := s.itemUC.List(ctx, &ItemListIn{
	...
	return &systempb.ListItemsResponse{
		...
		List: utils.SliceMap(out.List, toProtoItem),
	}, nil
}
...
func toProtoItem(a *Item) *systempb.Item {
