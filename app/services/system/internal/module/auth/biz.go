...
import (
	...
	authtoken "cyber-ecosystem/shared-go/kratos/security/auth"
...
type AuthUC struct {
	shared.UC
	userRP  user.UserRP // cross-module port read (DAG: auth → user)
	tokenRP TokenRP
}

func NewAuthUC(logger *slog.Logger, tm shared.Transaction, userRP user.UserRP, tokenRP TokenRP) *AuthUC {
	return &AuthUC{
...
func (uc *AuthUC) Login(ctx context.Context, email, password string) (*TokenPair, error) {
	...
	sid, err := authtoken.GenerateToken()
	if err != nil {
		return nil, err
	}
